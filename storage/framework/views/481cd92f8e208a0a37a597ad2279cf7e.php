<div style="max-width:680px;margin:0 auto;padding:32px;font-family:Arial,sans-serif;font-size:13px;color:#111">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;padding-bottom:18px;border-bottom:2px solid #4F46E5">
    ...
    <div style="text-align:right">
      ...
      <div style="font-size:12px;color:#6B7280;margin-top:4px"><?php echo e($invoice->no_invoice); ?></div>
    </div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:24px">
    <div>
      ...
      <h3 style="margin:0 0 4px;font-size:15px;font-weight:700"><?php echo e($invoice->order->customer->nama); ?></h3>
      <span style="font-size:12px;color:#6B7280"><?php echo e($invoice->order->customer->no_telp ?? ''); ?><br><?php echo e($invoice->order->customer->alamat ?? ''); ?></span>
    </div>
    <div>
      ...
      <h3 style="margin:0 0 4px;font-size:15px;font-weight:700"><?php echo e($invoice->no_invoice); ?></h3>
      <span style="font-size:12px;color:#6B7280">Tanggal: <?php echo e($invoice->created_at->format('d M Y')); ?><br>Order: #<?php echo e(str_pad($invoice->order_id,3,'0',STR_PAD_LEFT)); ?></span>
    </div>
  </div>
    ...
    <tbody>
      <tr>
        <td style="padding:12px 14px;border-bottom:1px solid #F3F4F6">
          <strong><?php echo e($invoice->order->jenis_laundry); ?></strong>
          <?php if($invoice->order->catatan): ?><br><small style="color:#6B7280"><?php echo e($invoice->order->catatan); ?></small><?php endif; ?>
        </td>
        <td style="padding:12px 14px;border-bottom:1px solid #F3F4F6"><?php echo e($invoice->order->total_berat); ?> kg</td>
        <td style="padding:12px 14px;border-bottom:1px solid #F3F4F6">Rp <?php echo e(number_format($invoice->order->harga_per_kg,0,',','.')); ?></td>
        <td style="padding:12px 14px;border-bottom:1px solid #F3F4F6;text-align:right;font-weight:700">Rp <?php echo e(number_format($invoice->subtotal,0,',','.')); ?></td>
      </tr>
    </tbody>
  </table>
  ...
  <div style="display:flex;justify-content:flex-end">
    <div style="background:#F9FAFB;border-radius:10px;padding:16px 24px;min-width:220px">
      <div style="display:flex;justify-content:space-between;gap:32px;margin-bottom:6px;font-size:13px">
        <span>Subtotal</span><span>Rp <?php echo e(number_format($invoice->subtotal,0,',','.')); ?></span>
      </div>
...
<?php /**PATH /Users/mac/Downloads/ql/resources/views/admin/invoice/partial.blade.php ENDPATH**/ ?>